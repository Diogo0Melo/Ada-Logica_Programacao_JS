...
function cadastrarAluno() {
    ...
    if (tentativas) {
        tentativas = false
        console.error(`Excesso de tentativas, programa encerrando!\nO aluno não foi cadastrado.`);
        return
    }
...
function pegarClassificacao(turma) {
    for (let i = 0; i <= 3; i++) {
        if (i === 3 || tentativas) return tentativas = true
        const classificacao = prompt("Informe a classificação do aluno [A, B, C ou D]").toUpperCase().trim();
        if (classificacao !== "A" && classificacao !== "B" && classificacao !== "C" && classificacao !== "D") {
            console.warn(`A classificação ${classificacao} é inválida.\nTente novamente.`);
            continue
        }
        else if (classificacao === "A" || classificacao === "D"  && turmas[turmas.findIndex(t => t.idTurma == turma)].alunos.find(c => c.classificacao !== classificacao)) {
            console.warn(`Alunos com classificação ${classificacao} não podem estar na mesma turma que alunos com a classifição B ou C.\nTente novamente.`);
            continue
        }
        else if (classificacao === "B" || classificacao === "C" && turmas[turmas.findIndex(t => t.idTurma == turma)].alunos.find(c => c.classificacao !== classificacao)) {
